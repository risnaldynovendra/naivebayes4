...
# from indonesian_sentiment import IndonesianSentimentAnalysis
...
class TweetSentimentApp:
    ...
    def clean_text(self, text):
        ...
        # Menggunakan modul Sastrawi untuk melakukan stemming pada teks bahasa Indonesia
    ...
    def analyze_sentiment(self, text):
        # Sastrawi tidak menyediakan analisis sentimen bawaan, sehingga Anda perlu menyesuaikan metode ini
        # Contoh sederhana: Jika terdapat kata positif, dianggap positif; jika terdapat kata negatif, dianggap negatif
        if any(word in text.lower() for word in self.positive_words):
            sentiment_score, sentiment_tag = 0.5, 'positive'
        elif any(word in text.lower() for word in self.negative_words):
            sentiment_score, sentiment_tag = -0.5, 'negative'
        else:
            sentiment_score, sentiment_tag = 0, 'neutral'
    ...
    def get_sentiment_score(self, tweet):
        if isinstance(tweet, str):
            cleaned_text = self.clean_text(tweet)
            sentiment_score, _ = self.analyze_sentiment(cleaned_text)
    ...
    def analyze_tweets_sentiment(self, dataframe):
        ...
        for index, row in dataframe.iterrows():
            ...
            # Use Sastrawi stemmer to get the root form of each word
            stemmed_words = stemmer.stem(tweet).split()
    ...
    def sentiment_count(self, dataframe):
        # Group the dataframe by sentiment tag and count the number of tweets
        sentiment_count_df = dataframe['sentiment_tag'].value_counts().reset_index()
        sentiment_count_df.columns = ['sentiment_tag', 'tweet_count']

        # Find the sentiment with the highest tweet count
        max_sentiment = sentiment_count_df.loc[sentiment_count_df['tweet_count'].idxmax()]

        # Print the sentiment and highest tweet count
        max_sentiment_tag = max_sentiment['sentiment_tag']
        max_tweet_count = max_sentiment['tweet_count']
        print(f"Overall Sentiment Count: {max_sentiment_tag}, Tweet Count: {max_tweet_count}")

        # Plot tweet count for each sentiment
        plt.bar(sentiment_count_df['sentiment_tag'], sentiment_count_df['tweet_count'], color=['red', 'green', 'blue'], width=0.5)
        ...
        for i, count in enumerate(sentiment_count_df['tweet_count']):
    ...
    def plot_word_frequency_by_user(self, df):
        ...
        for index, row in df.iterrows():
            ...
            cleaned_tweet = self.clean_text(tweet)  # Gunakan metode clean_text yang sudah diperbaiki sebelumnya

            for word in cleaned_tweet.split():
                if word in self.positive_words or word in self.negative_words:
                    sentiment = 'positive' if word in self.positive_words else 'negative'

                    user_word_frequency.setdefault(username, {'positive': {}, 'negative': {}})
                    user_word_frequency[username][sentiment][word] = user_word_frequency[username][sentiment].get(word, 0) + 1
    ...
    def calculate_sentiment_percentages(self, df):
        ...
        for index, row in df.iterrows():
            ...
            tweet = stopword_remover.remove(tweet)
...
# =============================================================================================
    ...
    def get_agitation_keywords(self, tweet):
        ...
        for word in self.agitative_words:
            if re.search(r'\b' + re.escape(word) + r'\b', tweet, re.IGNORECASE):
    ...
    def predict_single_user_sentiment(self, user_input):
        ...
        sentiment_score = self.get_sentiment_score(cleaned_input)
        sentiment_tag = self.sentiment_label(sentiment_score)
        predicted_sentiment_label = self.predict_sentiment(cleaned_input)
